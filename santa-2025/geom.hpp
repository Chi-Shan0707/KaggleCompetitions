...
public:
    ...
    void translate(const Vec2<T>& t) override { for (auto &p: pts) p = p + t; } // EN: Add translation to each vertex. CN：逐顶点加平移。
    void rotateAround(T deg, const Vec2<T>& origin) override { // EN: Rotate all vertices around origin. CN：围绕 origin 旋转所有顶点。
        for (auto &p: pts) { auto rel = p - origin; rel = rotate(rel, deg); p = origin + rel; } // EN: Shift to origin, rotate, shift back. CN：平移到原点→旋转→移回。
    }
    static std::pair<T,T> projectOnAxis(const std::vector<Vec2<T>>& v, const Vec2<T>& axis){ // EN: Project polygon onto axis for SAT. CN：将多边形投影到轴上用于 SAT。
        T minP = dot(v[0], axis), maxP = minP; // EN: Initialize min/max projection. CN：初始化投影最小/最大值。
        for(size_t i=1;i<v.size();++i){ T d=dot(v[i],axis); minP=std::min(minP,d); maxP=std::max(maxP,d);} return {minP,maxP}; // EN: Update extremes. CN：更新投影范围。
    }
};

template <typename T>
class ConvexPolygon : public PolygonBase<T> { // EN: Convex polygon with SAT overlap test. CN：带 SAT 检测的凸多边形。
public:
    using PolygonBase<T>::PolygonBase; // EN: Inherit constructors. CN：继承构造函数。
    bool overlaps(const Shape<T>& other) const override { // EN: Polymorphic collision test. CN：多态碰撞检测。
        auto *oConvex = dynamic_cast<const ConvexPolygon<T>*>(&other); // EN: Try cast to convex polygon. CN：尝试转换为凸多边形。
        if(!oConvex) return other.overlaps(*this); // EN: Defer if different type. CN：不同类型→交由对方处理。
        const auto &A = this->vertices(); // EN: This polygon's vertices. CN：当前多边形顶点。
        const auto &B = oConvex->vertices(); // EN: Other polygon's vertices. CN：另一多边形顶点。
        auto testAxes = [&](const std::vector<Vec2<T>>& v){ // EN: Lambda to test axes from vertex list. CN：测试给定顶点集合的所有边法向。
            for(size_t i=0;i<v.size();++i){ auto e = v[(i+1)%v.size()] - v[i]; auto axis = perp(e); // EN: Edge & its perpendicular axis. CN：边与其法向轴。
                auto [aMin,aMax] = PolygonBase<T>::projectOnAxis(A,axis); // EN: Projection range of A. CN：A 在轴上的投影区间。
                auto [bMin,bMax] = PolygonBase<T>::projectOnAxis(B,axis); // EN: Projection range of B. CN：B 在轴上的投影区间。
                if(aMax < bMin || bMax < aMin) return false; } // EN: Disjoint intervals ⇒ separating axis. CN：区间不重叠→找到分离轴。
            return true; };
        return testAxes(A) && testAxes(B); // EN: Must pass all axes from both polygons. CN：需通过双方所有边法向。
    }
};

template <typename T>
ConvexPolygon<T> makeRegularPolygon(int sides, T radius, const Vec2<T>& center={0,0}, T rotationDeg=0){ // EN: Build regular polygon. CN：构造正多边形。
    std::vector<Vec2<T>> v; v.reserve(sides); // EN: Reserve memory. CN：预留空间。
    for(int i=0;i<sides;++i){ T ang = rotationDeg + (T)i * (T)360 / (T)sides; // EN: Angle for vertex i. CN：第 i 个顶点角度。
        auto p=Vec2<T>{radius,0}; p=rotate(p,ang); v.emplace_back(center+p);} // EN: Rotate radius vector & translate. CN：旋转半径向量后平移到中心。
    return ConvexPolygon<T>(std::move(v)); // EN: Return constructed polygon. CN：返回构造好的多边形。
}
...
T boundingSquareSide(const std::vector<ConvexPolygon<T>>& polys){
    ...
    for(auto &poly: polys){ // EN: Iterate polygons. CN：遍历多边形。
        for(auto &p: poly.vertices()){ // EN: Iterate vertices. CN：遍历顶点。
            if(first){minx=maxx=p.x; miny=maxy=p.y; first=false;} // EN: Initialize with first vertex. CN：用首顶点初始化范围。
            else { minx=std::min(minx,p.x); miny=std::min(miny,p.y); maxx=std::max(maxx,p.x); maxy=std::max(maxy,p.y);} // EN: Update bounds. CN：更新边界值。
        }
    }
    T w = maxx - minx; T h = maxy - miny; return std::max(w,h); // EN: Square side = max(width,height). CN：正方形边长为宽高较大者。
}
